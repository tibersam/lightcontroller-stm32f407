        ...
        while True:
            ...
            if counter == 10:
                ...
                raise ControllerUnresponsive(
                    "Controller is not responding in the way expected"
                )
            counter = counter + 1
...
class StopLightController(Exception):
    pass
...
def lightcontrollerbackend(
    commandqueue, abortqueue, returnqueue, path_to_serial=None, timeout=1
):
    ...
    try:
        while True:
            ...
            if not Check_abortqueue(
                abortqueue, commandqueue, returnqueue, delay=command["delay"]
            ):
...
class LightController:
    ...
    def set_rgb_limit(
        self, r: int, g: int, b: int, offset: int, length: int, mode=-1, delay=0
    ):
        if mode == -1:
            mode = self.mode
        if r > 255 or g > 255 or b > 255:
            raise ValueError("RGB values need to be between 0-255!")
        s = "set limit {r} {g} {b} {offset} {length} {mode}\r".format(
            r=r, g=g, b=b, offset=offset, length=length, mode=mode
        )
        self.write(s, delay=delay)
